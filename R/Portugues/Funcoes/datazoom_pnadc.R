...
### Funcao que apenas pega os dados brutos da PNAD-C
### para um numero arbitrario de periodos e devolve
### uma lista com uma dataframe para cada um deles
### Para que a funcao funcione, baixar nao soh
### os arquivos.txt com os dados, como o arquivo
### .xls com o dicionario da pesquisa.
...
datazoom_pnadc <- function(diretorio_dados,
  ...
  # Forma uma lista de trimestres e de anos

  datas <- list(...)


  # Mensagem de erro caso ano e trimestres escolhidos n\u{fffd}o
  # tenham mesmo comprimento
  if (any(map(datas, length) != 2)) {stop('Escolha o mesmo número de anos e trimestres', call. = FALSE)}
  trimestre <- datas %>% map( ~ .x[[1]])
  ano <- datas %>% map( ~ .x[[2]])
  # Mensagem de erro no caso de escolha errada de ano
  ...
  # Salva as descricoes numa lista. O nome de cada elemento da lista  vai ser dado
  # # pelo codigo
  ...
  # Guarda o numero de digitos usados para definir separadores
  # # de colunas
  ...
  # Separa variaveis de acordo com seu tipo: fatores, caracteres e
  # numericas
  ...
  fatores = c('Ano', 'Trimestre', 'UF', 'Capital', 'RM_RIDE',
        ...
        # Carrega cada uma das bases de dados originais usando a lista
        # de caminhos dada por filepath_in, al\u{fffd}m de fazer a separacao em colunas
        ...
        # Nomeia cada elemento da lista de bases de dados de acordo com a data
        # da pesquisa

        names(DF) = paste0('PNADC_', trimestre, ano)

        # Converte vari\u{fffd}veis de acordo com seus tipos, al\u{fffd}m de gerar as
        # duas colunas de identificadores, colocadas como as primeiras na nova base
...
### Funcao auxiliar usada para elaborar o painel
### Nao rodar ela com argumento algum - apenas deixar carregado no
### R
...
#### Funcao que permite ao usuario montar o painel a partir
#### de dados ja no R (basta fazer build_data = FALSE),
#### ou primeiro fazendo o que datazoom_pnadc faz de carregar
#### o dado bruto (fazendo build_data = TRUE)
#### Se for escolhido TRUE, o usuario
#### eh obrigado a forncecer tambem os argumentos 'local_dados' e 'local_pastas'
#### para que se localizem os dados e o dicionario, alem de uma lista de datas
#### dentro do argumento 'periodos'.
#### Se escolher FALSE, o usuario apenas tem de fornecer,
#### no argumento 'dados_prontos", os dados da PNADC ja compilados no R
#### Ao final, independente de TRUE/FALSE, teremos uma lista de dataframes,
#### cada entrada sendo um painel
...
### Exemplos de implementacao das funcoes. Lembrando que, para fins de economia de tempo,
### configurei para que fossem lidas no maximo 1000 linhas por arquivo


PNADC_2012 <- datazoom_pnadc(diretorio_dados = './pnadcontinua',
                             diretorio_dicionario = './pnadcontinua/Dicionario_e_input',
                             c(1,2012), c(2,2012), c(3,2012), c(04,2012), c(1, 2013), c(2, 2013))



teste <- pnadc_painel_basico(build_data = TRUE,
                               local_dados = './pnadcontinua',
                               local_dicionario = './pnadcontinua/Dicionario_e_input',
                               periodos = list(c(1,2012), c(2,2012), c(3,2012), c(04,2012), c(1, 2013), c(2, 2013))
                               )

teste2 <- pnadc_painel_basico(build_data = FALSE,
                                dados_prontos = PNADC_2012)



save(datazoom_pnadc, loop, pnadc_painel_basico, file = './pnadcontinua/funcoes_painel.basico.RData')
