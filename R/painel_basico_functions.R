...
filters_data = function(incoming_dat){
  ...
  w_id_dom = character_dat %>%
    dplyr::bind_cols(
      ...
      id_dom = dplyr::group_indices(as.data.frame(dados_character), character_dat$UPA, character_dat$V1008, character_dat$V1014))
  
  #creates individual identifier
  #id_ind concatenates househol identifier (id_dom), birthday in Ymd format and gender
